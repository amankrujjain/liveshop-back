...
async function verifyToken(req, res, next) {
    try {
        ...
        if (!token || !username) {
            return res.status(401).json({ result: "Fail", message: "Authorization token or username is missing" });
        }
        ...
        if (user.role === "User") {
            ...
            return res.status(403).json({ result: "Fail", message: "You are not authorized to access this resource" });
        }
        ...
        if (!user.tokens.includes(token)) {
            return res.status(401).json({ result: "Fail", message: "Invalid token or session has expired, please log in again" });
        }
        ...
        if (error.name === 'TokenExpiredError') {
            return res.status(401).json({ result: "Fail", message: "Session expired, please log in again" });
        }

        // Handle invalid or other errors related to token verification
        console.error("Token verification error:", error.message);
        return res.status(401).json({ result: "Fail", message: "You are not authorized to access this resource" });
    }
}
//Payment API
...
app.put("/user/:_id", verifyToken, upload.single('pic'), async (req, res) => {
    try {
        ...
        if (error.keyValue) {
            return res.status(401).send({ result: "Fail", message: "User Name Must be Unique" });
        } else if (error.errors) {
            const errorMessages = Object.values(error.errors).map(err => err.message);
            return res.status(401).send({ result: "Fail", message: errorMessages.join(", ") });
        } else {
...
app.post("/login", async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(404).send({ result: "Fail", message: "Invalid Username or Password" });
        }
        ...
        if (user.tokens.length < 3) {
            ...
            res.status(401).send({ result: "Fail", message: "You are already logged in from 3 devices. Please log out from another device to log in here." });
        }
    } catch (error) {
...
app.post('/register-webauthn/verify', async(req,res)=>{
    try {
        ...
        res.status(500).send({ result: "fail", message: "Internal Server Error during registration verification" });
    };
})
...
app.post('/webauthn/login', async(req, res) => {
    try {
        ...
        if (!user.webAuthnCredentials || user.webAuthnCredentials.length === 0) {
            return res.status(404).send({ result: "Fail", message: "No WebAuthn credentials found for this user" });
        }
        ...
        res.status(500).send({ result: "fail", message: "Internal Server Error during login start" });
    }
});
...
app.post('/login-webauthn/verify', async(req,res)=>{
    try {
        ...
        const expectedChallenge = req.session.challenge;
        const credential = user.webAuthnCredentials.find(cred => cred.id === authResponse.id);
